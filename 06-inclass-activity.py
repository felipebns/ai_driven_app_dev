...
ticker2 = st.sidebar.text_input("Comparasion stock", value="SPY").upper()
...
if run:
    ...
    with st.spinner(f"Fetching {ticker2} data..."):
        df2, message2 = get_stock_data(ticker2, start, end)
    if df is not None:
        st.sidebar.success(message2)
    else:
        st.sidebar.error(message2)
    df2['MA'] = df2['Close'].rolling(window=moving_average).mean()
    df2['pct_change'] = df2['Close'].pct_change() * 100
    df2['normalized_close'] = (df2['Close'] / df2['Close'].iloc[0]) * 100
    ...
    with tab4:
        ...
        comparison_df = pd.DataFrame({
            ...
            ticker2: df2['normalized_close']
        })
        
        figure = px.line(comparison_df, 
                        y=[ticker, ticker2],
                        labels={'value': 'Normalized Close (Base 100)', 'variable': 'Ticker'},
                        title=f"{ticker} vs {ticker2} Performance (Base 100)")
    
        figure.update_layout(hovermode='x unified')
        with st.spinner("Creating comparison chart"):
            st.plotly_chart(figure, use_container_width=True)
        
        st.subheader("Summary Statistics (Normalized Data)")
        summary_stats = pd.DataFrame({
            'Ticker': [ticker, ticker2],
            'Min': [
                f"{df['normalized_close'].min():.2f}",
                f"{df2['normalized_close'].min():.2f}"
            ],
            'Max': [
                f"{df['normalized_close'].max():.2f}",
                f"{df2['normalized_close'].max():.2f}"
            ],
            'Final Normalized Value': [
                f"{df['normalized_close'].iloc[-1]:.2f}",
                f"{df2['normalized_close'].iloc[-1]:.2f}"
            ]
        })
